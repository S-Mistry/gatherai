...
1. **Copy leaks developer internals into user surfaces.** "WebRTC + server-minted secrets," "scaffolded routes," "analysis jobs queued," "transcript-only MVP," "Supabase RLS" — all visible to users. The worst offender is a participant-facing error that reads "*Realtime credentials are not configured yet. The participant flow is scaffolded and ready for env wiring.*" (`components/participant/interview-shell.tsx:114`).
...
The goal: **keep the visual DNA** (warm off-white, terracotta primary, Montserrat, frosted panels) but make the consultant feel like a senior colleague just handed them a notebook, and make the participant feel like a thoughtful person is listening — not an AI running a queue.
...
5. **Labor-illusion / progressive disclosure.** (Nielsen 4 & 6) — When we provision the realtime session, *show work happening* (mic check → secure channel → ready). When we auto-save a transcript segment, *say so*, don't hide it.
6. **"Talk like a human."** (Mailchimp Content Style Guide, widely adopted) — Contractions, short sentences, no passive voice, no product name in copy the user didn't ask for.
...
| "GatherAI turns pre-workshop discovery into a structured voice workflow: share one public link, run consistent AI interviews, and surface themes, contradictions, and workshop agenda inputs backed by transcript evidence." | "Share one link. A thoughtful AI interviewer listens to each stakeholder for 10–15 minutes and hands you back themes, contradictions, and an agenda — every line traceable to a real quote." |
...
#### Consultant home (`app/app/page.tsx`)
...
#### New project (`app/app/projects/new/page.tsx`)

| Before | After |
|---|---|
| H1: "Configure the next workshop interview project" | "New discovery project" |
| "The MVP prioritizes configuration that drives coverage: objective, areas of interest, required questions, duration cap, and anonymity mode." | "Set the objective, pick the topics, and draft the questions. You can edit everything after sharing the link." |
...
#### Participant entry (`app/i/[linkToken]/page.tsx`)

| Before | After |
|---|---|
| Badge: "Public participant link" | "For you" |
| H1: "Share perspective before the workshop" | "Before the workshop — a 15-minute chat." |
| "This short AI interview helps the consultant understand pain points, contradictions, and workshop expectations before live facilitation begins." | "{consultantName} is running a workshop with your team. Before we meet, they'd love to hear what's working, what isn't, and what you'd change. I'm an AI that'll ask a few questions and listen — for about 15 minutes." |
...
#### Interview shell (`components/participant/interview-shell.tsx`)

| Before | After |
|---|---|
| "Review the disclosure, then start when you are ready." | "Take a breath. Press start when you're ready." |
| "Creating your session and preparing voice transport..." | "Setting up your microphone…" (with 3-step indicator: ✓ Session created · ◔ Secure channel · ○ Mic ready) |
...
Placed above the H1, `text-xs text-muted-foreground`, with `/` separators and chevron icons. Implements Don Norman's "where am I, how did I get here."
...
### C.2 New components (all in `components/ui/`)
...
| `Skeleton` | Loading placeholder | `animate-pulse bg-muted/60 rounded-2xl`. Stand-in for cards and list rows. |
| `EmptyState` | Zero-data screens | Icon + title + body + one CTA. Centered inside the parent panel. |
| `Confirm` (dialog) | Destructive action gate | Radix dialog wrapped, uses `.panel` styling. "Exclude this session? It won't count toward synthesis. [Cancel] [Exclude]" |
| `VoiceStatus` | Agent state indicator | Three states: *listening* (pulsing dot), *thinking* (animated 3-dot), *speaking* (bar). Replaces raw status message. The conversation's only structural UI element. |
...
- Add a second row: **"Needs your attention"** — stacked list of flagged sessions across all projects, each with a "Review" button. Evidence-first: the consultant's real job is curating.
...
- Each theme in synthesis is a click target that opens a side-drawer with the transcript segments pinned to it. This is the **Teresa Torres "evidence-linked" principle made visible**.
...
- Live preview panel on the right at `lg:` breakpoint: renders a compact mock of what the participant will see. Removes the guesswork from anonymity mode.
...
- **Pre-start state**: hero with `consultantName`, project name, "About 15 minutes." Below it, the rewritten **"What happens to my voice?"** card with three reassurance lines and a Phosphor `ShieldCheck` icon. One primary CTA: **"Start when ready"** — full-width pill, `size="lg"`, terracotta. No secondary buttons (anything else is a distraction at this moment).
- **On Start tap**: trigger `getUserMedia({ audio: true })` immediately. While the browser permission prompt is up, the page replaces the CTA area with a quiet line: *"Allow microphone access to begin."* If denied, surface a recoverable error: *"We need microphone access to talk. Open your browser settings to allow it, then try again."*
- **On grant**: the realtime channel connects silently in the background. The hero card morphs into the **live conversation surface** (no page navigation):
  ...
  - Agent's current spoken text rendered above the indicator (last 1–2 sentences only, fades older lines), aiding accessibility and noisy environments.
  - Timer in the corner ("4 / ~15 min") that **only starts after the participant has said 'ready'** (or otherwise begun answering), not at page load or mic grant.
  - Two controls only: **Pause** (toggles agent listening) and **I'm done**.
...
- Verify dark-mode muted text passes WCAG AA on `--card` (current `oklch(0.74 0.012 95.1)` on `oklch(0.255 0.018 247.2)` — run once, adjust if needed).
...
1. **Realtime provisioning** — replace opaque "Creating your session…" with a 3-step checklist that animates: *Session ready → Secure channel open → Microphone linked*. Labor-illusion principle.
2. **Auto-save transcript events** — a quiet "Saved" ghost-toast in the corner every time a segment lands server-side. For the consultant side, not the participant.
3. **Exclusion is reversible** — every exclude action fires a 6-second toast with "Undo." This lowers stakes and increases trust.
4. **Synthesis freshness pill** — "Updated 3 min ago" next to the synthesis title; click = re-run. Sets expectation before the consultant clicks.
...
6. **Session completion sound** — very soft acknowledgment chime on participant completion (toggleable, respects `prefers-reduced-motion` and starts muted — a11y).
...
- `gather/components/participant/interview-shell.tsx` — major rewrite: 3-stage flow, hide IDs, replace status text with `VoiceStatus`, new end screen.
...
- `docs/voice-and-tone.md` — source of truth for copy voice, banned words, CTA verb table. Regenerated through the existing `scripts/build-agents.mjs` pipeline by referencing it from `docs/agent-guide.md`.
...
## F. Sequencing (recommended rollout order)

1. **Copy pass** (half-day) — highest value, lowest risk. Touches only `.tsx` strings in existing files. Immediate improvement.
2. **Participant flow redesign** (1–2 days) — the most important user to get right; the most anxiety and the biggest copy cleanup.
3. **New UI primitives** (1 day) — Breadcrumb, Toast, Skeleton, EmptyState, Confirm, RelativeTime.
4. **Consultant dashboard density + evidence drawer** (1–2 days) — applies primitives; delivers the "senior colleague" feel.
5. **Accessibility + motion-reduce + contrast verify** (half-day).

---

## G. Verification

1. **Visual QA**: `npm --prefix gather run dev`, walk both flows end-to-end in Chrome at 375px, 768px, 1280px. Toggle light/dark with the existing `d` hotkey. Confirm no `.panel` padding looks empty at new densities.
2. **Copy QA**: grep the codebase for every banned word (`scaffold|MVP|WebRTC|credentials|transport|bootstrap|job queue|RLS|metadata prompts`). Zero hits in `app/` and `components/` outside comments.
...
5. **a11y**: run `npx @axe-core/cli http://localhost:3000` on each route. Verify keyboard-only navigation works end-to-end (Tab order, focus rings, `aria-live` announces status).
