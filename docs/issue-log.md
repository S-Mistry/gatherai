...
- Problem: On mobile browsers the participant shell always showed "We need microphone access to talk. Open your browser settings to allow it, then try again." even when the real cause was Brave Shields blocking `navigator.mediaDevices`, an in-app WebView (Instagram, WhatsApp, LinkedIn, etc.) without mic support, a prior deny that iOS Safari silently honors, an insecure context, or an in-use microphone — and the agent audio element was not primed inside the user-gesture window, so iOS Safari could refuse to autoplay it after the async connect chain.
- Cause: `navigator.mediaDevices.getUserMedia` was called without guarding for insecure contexts or missing APIs, its failures were swallowed into one message regardless of the `DOMException` name, there was no preflight detection for Brave Shields or in-app WebViews before the user clicked Start, the only recovery from an error was a full page reload, and `agentAudio.play()` ran several `await`s after the click.
- Avoid: Treat participant mic acquisition as mobile-first. Run a mount-time `detectMicSupport` preflight that branches on `window.isSecureContext`, `navigator.mediaDevices`, Brave UA / `navigator.brave.isBrave()`, common in-app WebView UA patterns, and the Permissions API; render an actionable notice (with Copy link + retry) instead of the Start button when support is missing. When Start is clicked, prime the agent `<audio>` element synchronously *before* any `await`, map DOMException names to targeted copy with iOS- vs Android-specific recovery steps, and offer an in-page Try-again button so users don't need to reload after granting permission.
- Fix/Check: On iPhone Safari, Android Chrome, and Brave on both, open the participant link, tap Start, and confirm the native mic prompt appears first-try and Mia's voice plays back. Set Brave Shields to Aggressive and confirm the page shows "Brave Shields is blocking the microphone" before Start. Open the link from inside WhatsApp or Instagram and confirm the WebView notice appears with Copy link + Open in Safari/Chrome guidance. Deny the mic prompt and confirm the deny-specific copy mentions the correct settings path and the Try-again button retries without a reload. Run `npm --prefix gather run test:fixtures` to keep `mic-support.test.ts` green.
