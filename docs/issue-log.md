...
- Problem: On mobile browsers the participant shell always showed "We need microphone access to talk. Open your browser settings to allow it, then try again." even when the real cause was insecure context, an unsupported browser, or an in-use microphone, and agent audio sometimes failed to autoplay on iOS Safari.
- Cause: `navigator.mediaDevices.getUserMedia` was called without guarding for insecure contexts or missing APIs, its failures were swallowed into one message regardless of the `DOMException` name, and the realtime transport created its own detached audio element, which iOS Safari's autoplay policy can refuse after async setup.
- Avoid: Treat participant mic acquisition as mobile-first. Check `window.isSecureContext` and `navigator.mediaDevices` before calling `getUserMedia`, map each DOMException name to a targeted message (denied, not-found, in-use, insecure, unsupported) with iOS- vs Android-specific recovery steps, log the raw error to the console, and own the agent `<audio>` element in the DOM so it stays part of the user-gesture chain.
- Fix/Check: On an iPhone and on Android, open the participant link, grant mic access, and confirm the interview starts and Mia's voice plays. Revoke mic permission, retry, and confirm the permission-denied copy mentions the correct settings path. Load the page over `http://` on a LAN IP and confirm the shell shows the insecure-context message instead of a generic permission error.
