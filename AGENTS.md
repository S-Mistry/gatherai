...
- Each consultant page renders its own `<ConsultantAppBar crumb={…} rightSlot={…} />` at the top followed by its own outer `<div style={{padding, maxWidth, margin: '0 auto'}}>`. The consultant layout (`app/app/layout.tsx`) only provides the auth gate and `<ConsultantSessionProvider>`; it does not paint any chrome. See `STYLE_GUIDE.md` §6.2 for canonical paddings (Dashboard 1280, Synthesis 1320, Session review 1320, DeepInterview 1320, FeedbackInterview 760, Completion 640).
- Breadcrumbs deepen with route depth — `Workspace / Projects`, `Workspace / {project}`, `Workspace / {project} / {respondent}`. Status pills and overflow menus go into the AppBar `rightSlot`, not into a second sticky row.
